...
import {IComment, useFetchReviewComments} from "@/hooks/useFetchReviewComments";
...
export const Posts = ({reviews}: {reviews: IReview}) => {
    const reviewDoc = useFireStore(Collections.REVIEWS);
    const [openComment, setOpenComment] = useState<boolean>(false);
    const {countComment, loading} = useFetchReviewComments();
    ...
    useEffect(() => {
        (async () => {
            const count = await countComment(reviews.id!);
    ...
    const likeHandler = async () => {
        const  likeResult = await reviewDoc.updateDocument(reviews.id!, {
            likes: reviews.likes ? (reviews.likes + 1) : 1,
        })
    ...
    const disLikeHandler = async () => {
        const  disLikeResult = await reviewDoc.updateDocument(reviews.id!, {
            dislikes: reviews.dislikes ? (reviews.dislikes + 1) : 1,
        })

        if (disLikeResult instanceof Error) {
            console.error(disLikeResult.message);
        }
    }
    ...
    return (
        <>
            <div className='w-full my-2 lg:my-3 border-b-[1px] pb-3 border-b-[#D9D9D9]'>
                        ...
                        <div className='flex flex-row space-x-2'>
                            <div className='flex flex-row justify-center items-center gap-1'>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"
                                     stroke-width="1.5"
                                     stroke="#0D2159" className="w-4 h-4 text-input"
                                     onClick={likeHandler}
                                >
                                ...
                                <p className='text-title-xsm text-[#0D2159]'>{reviews.likes ?? 0}</p>
                            </div>
                            <div className='flex fle-row justify-center items-center gap-1'>
                                ...
                                <p className='text-title-xsm text-[#0D2159]'>{reviews.dislikes ?? 0}</p>
                            </div>
                            <div className='flex fle-row justify-center items-center gap-1'>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"
                                     stroke-width="1.5"
                                ...
                                <p className='text-title-xsm text-[#0D2159]'>{commentCount}</p>
                            </div>
                        </div>
                    </div>
                </section>
            </div>
            {openComment && <MakeComment reviewId={reviews.id!} closeComment={() => setOpenComment(false)} />}
        </>
    )
}

const MakeComment: FC<{reviewId: string; closeComment: Function}> = ({reviewId, closeComment}) => {
    const {postNewComment, loading} = useFetchReviewComments();
    ...
    return (
        <div className='flex flex-row p-3 w-full border-b-[1px] border-b-[#D9D9D9]'>
            <input value={newComment.comments} onChange={(e: ChangeEvent<HTMLInputElement>) =>  setNewComment(c => ({...c, comments: e.target.value}))} className='border-0 outline-0  text-[#1E1E1E] flex-grow bg-transparent' placeholder='Add a comment'/>
            <Button onClick={handler} variant='default' size='sm' className='text-white w-[100px] uppercas capitalizee'>{loading == 'fetching' ?  <Spinner/> : 'Post'}</Button>
        </div>
    )
}